...
# Adapted from scitex-ssh's .github/ci/publish-in-sif.sh (the fleet's
# established sibling convention) — only the TMPDIR label and package name in
# the trust-config comment / final echo changed; logic otherwise unchanged.
#
...
#
# REQUIRES a Trusted Publisher to already be configured on PyPI for
# (project=scitex-storage, owner=scitex-ai, repo=scitex-storage,
#  workflow=release.yml, environment=pypi). This is a ONE-TIME, PyPI-side
# (pypi.org/manage/) operator action — NOT something this script, or any CI
# job, can self-configure. If it is not yet registered, mint-token below
# returns an empty token and this script fails loud with PyPI's raw error body.
#
...
TMPDIR="/tmp/publish-scitex_storage-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
export TMPDIR
rm -rf "$TMPDIR"
...
echo "PUBLISH-OK: scitex-storage dist/* uploaded to PyPI via manual OIDC trusted publishing"
