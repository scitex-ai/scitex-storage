...
# exec-in-sif.sh). Builds scitex-storage's wheel + sdist into ./dist/.
# Adapted from scitex-ssh's .github/ci/build-in-sif.sh (the fleet's
# established sibling convention) — only the TMPDIR label and echoed package
# name changed; logic otherwise unchanged.
#
...
TMPDIR="/tmp/build-scitex_storage-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
export TMPDIR
rm -rf "$TMPDIR"
