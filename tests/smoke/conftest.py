"""Isolated environment for smoke tests.

Smoke tests drive the REAL installed ``scitex-storage`` console script in
a subprocess. Pointing ``HOME`` at a tmp dir proves the happy path never
depends on the developer's own user state — without mutating
``os.environ`` (no ``monkeypatch``; the env dict is passed explicitly to
``subprocess.run``).
"""
